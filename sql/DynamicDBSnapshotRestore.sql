...
WHEN @CreateOrRestore = 1 THEN
    ...
    ON     (NAME = N''PrimaryFile'', FILENAME = ''' + @Drive + ':\DB\Primary_Snapshot_' + @Database + '.ss'')
           ,(NAME = N''BlobsFile'', FILENAME = ''' + @Drive + ':\DB\Blobs_Snapshot_' + @Database + '.ss'')
           ,(NAME = N''InventoryFile'', FILENAME = ''' + @Drive + ':\DB\Inventory_Snapshot_' + @Database + '.ss'')
           ,(NAME = N''OrdersFile'', FILENAME = ''' + @Drive + ':\DB\Orders_Snapshot_' + @Database + '.ss'')' + CASE WHEN @Server <> 4 THEN ' ' WHEN @Server = 4 THEN + CHAR(10) + ',(NAME = N''SSD1File'', FILENAME = ''' + @Drive + ':\DB\SSD1_Snapshot_'+ @Database + '.ss'')' + CHAR(10) ELSE ' ' END +
